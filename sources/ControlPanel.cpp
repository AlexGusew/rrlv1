...
void ControlPanel::Initialize(int screenWidth, int screenHeight) {
  panelArea = {screenWidth * 2.0f / 3.0f, 0, screenWidth / 3.0f, (float)screenHeight};
...
void ControlPanel::UpdateInventoryScroll(const Player& player) {
  ...
  if (CheckCollisionPointRec(mousePosScreen, panelInventoryArea)) {
    ...
    float maxScroll = fmaxf(0.0f, (player.inventoryNodes.size() * NODE_INV_ITEM_HEIGHT) - panelInventoryArea.height);
...
void ControlPanel::UpdateGridCamera() {
  ...
  if (CheckCollisionPointRec(mousePosScreen, panelGridArea)) {
    Vector2 mousePosWorldBeforeZoom = GetScreenToWorld2D(mousePosScreen, panelCamera);
    panelCamera.zoom += GetMouseWheelMove() * 0.05f * panelCamera.zoom;
    panelCamera.zoom = Clamp(panelCamera.zoom, 0.2f, 3.0f);
    Vector2 mousePosWorldAfterZoom = GetScreenToWorld2D(mousePosScreen, panelCamera);
    panelCamera.target = Vector2Add(panelCamera.target, 
        Vector2Subtract(mousePosWorldBeforeZoom, mousePosWorldAfterZoom));
  }
}

void ControlPanel::UpdateNodeDragging(Player& player) {
  Vector2 mousePosScreen = GetMousePosition();
  Vector2 mousePosPanelGridWorld = GetScreenToWorld2D(mousePosScreen, panelCamera);
  ...
  if (CheckCollisionPointRec(mousePosScreen, panelArea)) {
    if (IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
      ...
      if (CheckCollisionPointRec(mousePosScreen, panelInventoryArea)) {
        for (size_t i = 0; i < player.inventoryNodes.size(); ++i) {
          Rectangle nodeRect = {
              ...
              panelInventoryArea.y + 5 + i * NODE_INV_ITEM_HEIGHT - inventoryScrollOffset,
      ...
      if (draggingNodeIndex == -1 && CheckCollisionPointRec(mousePosScreen, panelGridArea)) {
    ...
    if (IsMouseButtonDown(MOUSE_LEFT_BUTTON) && draggingNodeIndex != -1 && !draggingFromInventory) {
      // Update dragged node position
      if (draggingNodeIndex < (int)player.placedNodes.size()) {
        player.placedNodes[draggingNodeIndex]->panelPosition = mousePosPanelGridWorld;
      }
    }
...
void ControlPanel::UpdateNodeConnections(Player& player) {
  ...
  if (CheckCollisionPointRec(mousePosScreen, panelGridArea)) {
    for (auto& node : player.placedNodes) {
      if (node && CheckCollisionPointCircle(mousePosPanelGridWorld,
                                            node->panelPosition,
                                            NODE_UI_SIZE / 2.0f)) {
  ...
  if (IsMouseButtonReleased(MOUSE_RIGHT_BUTTON)) {
    if (connectingNodeFromId != -1 && toNodeForConnection) {
      ...
      if (fromNode && fromNode->id != toNodeForConnection->id) {
          ...
          if (toNodeForConnection->isStatType() ||
            ...
            if (toNodeForConnection->isActionType()) {
              ...
              for (size_t i = 0; i < fromNode->connectedToNodeIDs.size();) {
                BaseNode* existingTarget = player.GetPlayerNodeById(fromNode->connectedToNodeIDs[i]);
            ...
            if (std::find(fromNode->connectedToNodeIDs.begin(),
                          ...
                          toNodeForConnection->id) == fromNode->connectedToNodeIDs.end()) {
        ...
        else if (fromNode->isActionType() && toNodeForConnection->isActionType()) {
        ...
        else if (fromNode->isStatType() && toNodeForConnection->isActionType()) {
          if (std::find(fromNode->connectedToNodeIDs.begin(),
                        fromNode->connectedToNodeIDs.end(),
                        toNodeForConnection->id) == fromNode->connectedToNodeIDs.end()) {
            fromNode->connectedToNodeIDs.push_back(toNodeForConnection->id);
        ...
        // POWER to STAT/ACTION modifier connections
                 ...
                 (toNodeForConnection->isActionType() || toNodeForConnection->isStatType())) {
          // Remove existing POWER connections (POWER nodes can only connect to one target)
...
void ControlPanel::HandleNodePlacement(Player& player) {
  Vector2 mousePosScreen = GetMousePosition();
  if (CheckCollisionPointRec(mousePosScreen, panelGridArea) && 
      draggingNodeIndex >= 0 && draggingNodeIndex < (int)player.inventoryNodes.size()) {
    Vector2 mousePosPanelGridWorld = GetScreenToWorld2D(mousePosScreen, panelCamera);
...
void ControlPanel::HandleNodeRemoval(Player& player) {
  Vector2 mousePosScreen = GetMousePosition();
  if (CheckCollisionPointRec(mousePosScreen, panelInventoryArea) && 
      draggingNodeIndex >= 0 && draggingNodeIndex < (int)player.placedNodes.size()) {
    
    // Don't allow removing the CPU core node
    if (player.placedNodes[draggingNodeIndex]->getType() == NodeType::CPU_CORE) {
      return;
    }
...
void ControlPanel::Draw(const Player& player) {
  ...
  DrawText("CONTROL PANEL (TAB)", (int)(panelArea.x + 10), (int)(panelArea.y + 10), 20, WHITE);
...
void ControlPanel::DrawInventoryArea(const Player& player) {
  DrawText("Inventory:", (int)panelInventoryArea.x, (int)(panelInventoryArea.y - 25), 15, WHITE);
  ...
  BeginScissorMode((int)panelInventoryArea.x, (int)panelInventoryArea.y,
                   (int)panelInventoryArea.width, (int)panelInventoryArea.height);
  
  // Draw inventory nodes - add safety check
  if (!player.inventoryNodes.empty()) {
    for (size_t i = 0; i < player.inventoryNodes.size(); ++i) {
      if (!player.inventoryNodes[i]) continue; // Safety check for null pointers
...
void ControlPanel::DrawGridArea(const Player& player) {
  DrawText("System Grid:", (int)panelGridArea.x, (int)(panelGridArea.y - 25), 15, WHITE);
      ...
      if (toNode) {
        ...
        if (isDirected) {
          Vector2 dir = Vector2Normalize(Vector2Subtract(
              toNode->panelPosition, fromNode->panelPosition));
          if (Vector2LengthSqr(dir) > 0) {
            float arrowHeadOffset = (NODE_UI_SIZE / 2.0f) + 2.0f / panelCamera.zoom;
...
void ControlPanel::DrawNodes(const Player& player) {
  for (const auto& node : player.placedNodes) {
    if (!node) continue; // Safety check for null pointers
    ...
    if (node->isActive && (node->isStatType() || node->isPowerType() || node->getType() == NodeType::CPU_CORE))
      borderColor = YELLOW;
    else if (node->id == player.activeActionNodeId && node->isCurrentlyActiveEffect)
      borderColor = WHITE;
    else if (node->isActionType() && !node->isCurrentlyActiveEffect && player.activeActionNodeId == node->id)
    ...
    Vector2 textSzMeasure = MeasureTextEx(GetFontDefault(), node->name.c_str(), textSize, 1.0f);
  ...
  if (CheckCollisionPointRec(mousePosScreen, panelGridArea)) {
    ...
    for (const auto& node : player.placedNodes) {
      ...
      if (CheckCollisionPointCircle(worldMouse, node->panelPosition,
                                    NODE_UI_SIZE / 2.0f)) {
        int yOff = 5;
        Vector2 tooltipPos = {mousePosScreen.x + 10, mousePosScreen.y + yOff - 2};
        
        std::string descText = node->description;
        std::string line2Text = TextFormat("ID:%d Val:%.1f", node->id, node->value);
...
void ControlPanel::DrawScrollbar(const Player& player) {
  if (player.inventoryNodes.size() * NODE_INV_ITEM_HEIGHT > panelInventoryArea.height) {
    ...
    float scrollRatio = (contentHeight > panelInventoryArea.height && panelInventoryArea.height > 0)
                            ? (inventoryScrollOffset / (contentHeight - panelInventoryArea.height))
                            : 0;
    float scrollbarY = panelInventoryArea.y + scrollRatio * (panelInventoryArea.height - scrollbarHeight);
    DrawRectangle((int)(panelInventoryArea.x + panelInventoryArea.width - 8),
                  (int)scrollbarY, 5, (int)fmaxf(10.0f, scrollbarHeight), LIGHTGRAY);
  }
}