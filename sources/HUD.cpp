...
void HUD::DrawGameHUD(const Player& player, bool isPanelOpen, int screenWidth, int screenHeight) {
...
void HUD::DrawActiveAction(const Player& player, bool isPanelOpen, int screenWidth) {
  if (player.activeActionNodeId != -1) {
    ...
    if (activeNode && activeNode->isCurrentlyActiveEffect) {
      const char* text = TextFormat("ACTION: %s (%.1fs)", activeNode->name.c_str(),
...
void HUD::DrawMainMenu(int screenWidth, int screenHeight) {
  DrawText("ROBO ROGUELIKE: POWER NODES v2",
           (int)(screenWidth / 2.0f - MeasureText("ROBO ROGUELIKE: POWER NODES v2", 30) / 2.0f),
           (int)(screenHeight / 2.0f - 40), 30, WHITE);
  DrawText("Press ENTER to Start",
           (int)(screenWidth / 2.0f - MeasureText("Press ENTER to Start", 20) / 2.0f),
...
void HUD::DrawGameOver(int screenWidth, int screenHeight) {
  ...
  DrawText("Press ENTER to Main Menu",
           (int)(screenWidth / 2.0f - MeasureText("Press ENTER to Main Menu", 20) / 2.0f),
           (int)(screenHeight / 2.0f + 20), 20, LIGHTGRAY);
}